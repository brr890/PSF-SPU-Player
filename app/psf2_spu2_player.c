...
static unsigned playlist_add_folder(PlayerState *state, const char *folder)
{
    ...
    char *out_first_path, size_t out_first_size)
{
    ...
    if (is_directory_path(path)) {
        ...
        if (playlist_add_folder(state, path) == 0) {
            return 0;
        }
        if (out_first_path != NULL && out_first_size > 0 &&
...
static const wchar_t g_music_open_filter[] =
    L"PSF/archive files (*.psf;*.minipsf;*.psf2;*.minipsf2;*.zip;*.7z;*.rar;*.tar;*.tgz;*.tbz;*.tbz2;*.txz;*.lha;*.lzh)\0"
    L"*.psf;*.minipsf;*.psf2;*.minipsf2;*.zip;*.7z;*.rar;*.tar;*.tgz;*.tar.gz;*.tbz;*.tbz2;*.tar.bz2;*.txz;*.tar.xz;*.lha;*.lzh\0"
    L"PSF files (*.psf;*.minipsf;*.psf2;*.minipsf2)\0*.psf;*.minipsf;*.psf2;*.minipsf2\0"
    L"Archive files (*.zip;*.7z;*.rar;*.tar;*.tgz;*.tbz;*.tbz2;*.txz;*.lha;*.lzh)\0"
    L"*.zip;*.7z;*.rar;*.tar;*.tgz;*.tar.gz;*.tbz;*.tbz2;*.tar.bz2;*.txz;*.tar.xz;*.lha;*.lzh\0"
    L"All files (*.*)\0*.*\0";
...
static int open_psf_files_to_playlist(HWND hwnd, PlayerState *state, char *out_first_path, size_t out_first_size)
{
    ...
    if (GetOpenFileNameW(&ofn)) {
        ...
        if (*name == L'\0') {
            ...
            if (path[0] != '\0' && add_music_source_to_playlist(hwnd, state, path, first, sizeof(first))) {
                added += 1;
                if (out_first_path != NULL && out_first_size > 0) {
...
static void playlist_play_index(HWND hwnd, PlayerState *state, int index)
{
    ...
    if (state->playlist_play_in_progress) {
        if (index != state->playlist_current) {
    ...
    for (;;) {
        ...
        start_playback(state->hwnd != NULL ? state->hwnd : hwnd, state, path);
...
static void open_direct_file_dialog(HWND hwnd, PlayerState *state)
{
    ...
    if (open_psf2_file(hwnd, path, sizeof(path))) {
        remember_playlist_file(state, path);
        if (is_archive_path(path)) {
            if (archive_add_to_playlist(hwnd, state, path, NULL, 0)) {
                show_playlist_window(state->hwnd != NULL ? state->hwnd : hwnd, state);
            }
        } else {
...
static LRESULT CALLBACK playlist_listbox_wnd_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (msg == WM_KEYDOWN && state != NULL && wparam == VK_RETURN) {
        if ((lparam & (1l << 30)) == 0 && state->playlist_hwnd != NULL) {
            SendMessageA(state->playlist_hwnd, WM_KEYDOWN, wparam, lparam);
        }
        return 0;
    }
...
static void paint_player(HWND hwnd, HDC hdc, PlayerState *state, int gauges_only)
{
    ...
    paint_core_panel(hdc, &live, key_on_events, release_events, voice_mute_mask, voice_reverb_force_on_mask, voice_reverb_force_off_mask, voice_noise_force_on_mask, voice_noise_force_off_mask, voice_pmod_force_on_mask, voice_pmod_force_off_mask, gauge_env, gauge_vol_l, gauge_vol_r, 0, hide_inactive, env_color_index, lr_color_index, env_custom_color, lr_custom_color, CORE0_X, y, line_height, psf_version != 0x01u, psf_version, stopped_display, startup_track_dimmed, active_text_color, inactive_text_color, muted_text_color, gauge_background_color, gauge_border_color, gauges_only);
    if (psf_version != 0x01u) {
        paint_core_panel(hdc, &live, key_on_events, release_events, voice_mute_mask, voice_reverb_force_on_mask, voice_reverb_force_off_mask, voice_noise_force_on_mask, voice_noise_force_off_mask, voice_pmod_force_on_mask, voice_pmod_force_off_mask, gauge_env, gauge_vol_l, gauge_vol_r, 1, hide_inactive, env_color_index, lr_color_index, env_custom_color, lr_custom_color, CORE1_X, y, line_height, 1, psf_version, stopped_display, startup_track_dimmed, active_text_color, inactive_text_color, muted_text_color, gauge_background_color, gauge_border_color, gauges_only);
    }
        ...
        if (state != NULL) {
            for (i = 0; i < count; ++i) {
                ...
                if (DragQueryFileW(drop, i, path_w, MAX_PATH) != 0 &&
                    ...
                    if (is_directory_path(path)) {
                        remember_playlist_folder(state, path);
                        if (add_music_source_to_playlist(hwnd, state, path, NULL, 0)) {
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line, int show_cmd)
{
    ...
    while (GetMessageA(&msg, NULL, 0, 0) > 0) {
        ...
        if (state.playlist_hwnd != NULL &&
            ...
            if (msg.wParam == VK_RETURN && (msg.lParam & (1l << 30)) != 0) {
                continue;
            }
