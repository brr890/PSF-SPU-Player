...
{
 int core=ch/24;
 unsigned int bit=1u<<(ch%24);

 if((psf2log_peops2_reverb_force_off_masks[core]&bit)!=0)
  {
   s_chan[ch].bRVBActive=0;
  }
 else
 if(((s_chan[ch].bReverbL || s_chan[ch].bReverbR) && (spuCtrl2[core]&0x80)) ||
    (psf2log_peops2_reverb_force_on_masks[core]&bit)!=0)       // reverb possible?
  {
...
static void StoreREVERB(int ch,int ns)
{
 ...
 if((psf2log_peops2_reverb_force_off_masks[core]&bit)!=0) return;
 else
 if(iUseReverb==1) // -------------------------------- // Neil's reverb
  {
   const int force_on=(psf2log_peops2_reverb_force_on_masks[core]&bit)!=0;
   const int reverb_l=force_on?1:s_chan[ch].bReverbL;
   const int reverb_r=force_on?1:s_chan[ch].bReverbR;
   const int iRxl=(s_chan[ch].sval*s_chan[ch].iLeftVolume*reverb_l)/0x4000;
   const int iRxr=(s_chan[ch].sval*s_chan[ch].iRightVolume*reverb_r)/0x4000;
