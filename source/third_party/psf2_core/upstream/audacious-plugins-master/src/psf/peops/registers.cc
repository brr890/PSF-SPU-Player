...
{
 psf2log_ps1_text_log_enabled=enabled?1:0;
}

static int psf2log_ps1_trace_time(void)
{
 return sampcount<=1345050u;
}

static void psf2log_ps1_trace(const char *fmt,...)
{
 FILE *file;
 va_list args;
 char path[MAX_PATH];

 if(!psf2log_ps1_text_log_enabled || !psf2log_ps1_trace_time()) return;
#ifdef _WIN32
 if(GetModuleFileNameA(NULL,path,sizeof(path))!=0)
 {
  ...
  {
   slash[1]='\0';
   strncat(path,"psf_spu_player.log",sizeof(path)-strlen(path)-1);
  }
  else
  {
   snprintf(path,sizeof(path),"psf_spu_player.log");
  }
 }
 else
#endif
 ...
 fprintf(file,"[PS1SPU] sample=%u sec=%.2f ",sampcount,(double)sampcount/44100.0);
 va_start(args,fmt);
   ...
   switch(r&0x0f)
    {
     ...
     //------------------------------------------------// start
     case 6:
       psf2log_ps1_trace("write START ch=%02d val=%04X old=%05X",ch,val,
        s_chan[ch].pStart!=nullptr ? (unsigned)((s_chan[ch].pStart-spuMemC)>>3) : 0u);
     ...
     case 8:
       {
        ...
        psf2log_ps1_trace("write ADSR1 ch=%02d val=%04X",ch,val);
        //---------------------------------------------//
     ...
     case 10:
      {
       const u32 lval=val; // DEBUG CHECK
       psf2log_ps1_trace("write ADSR2 ch=%02d val=%04X",ch,val);
     ...
     //------------------------------------------------//
     case 0xE:                                          // loop?
       psf2log_ps1_trace("write LOOP ch=%02d val=%04X",ch,val);
...
*/
    case H_SPUMute1:
      if(val) psf2log_ps1_endx_mask&=~(u32)val;
      psf2log_ps1_trace("write ENDX lo clear=%04X mask=%06X",val,(unsigned)psf2log_ps1_endx_mask);
      break;
    //-------------------------------------------------//
    case H_SPUMute2:
      if(val) psf2log_ps1_endx_mask&=~((u32)(val&0xffu)<<16);
      psf2log_ps1_trace("write ENDX hi clear=%04X mask=%06X",val,(unsigned)psf2log_ps1_endx_mask);
      break;
    //-------------------------------------------------//
    //-------------------------------------------------//
    case H_SPUon1:
      psf2log_ps1_trace("write KON lo val=%04X endx=%06X",val,(unsigned)psf2log_ps1_endx_mask);
      SoundOn(0,16,val);
      break;
    //-------------------------------------------------//
     case H_SPUon2:
	// printf("Boop: %08x: %04x\n",reg,val);
      psf2log_ps1_trace("write KON hi val=%04X endx=%06X",val,(unsigned)psf2log_ps1_endx_mask);
      SoundOn(16,24,val);
      break;
    //-------------------------------------------------//
    case H_SPUoff1:
      psf2log_ps1_trace("write KOFF lo val=%04X",val);
      SoundOff(0,16,val);
      break;
    //-------------------------------------------------//
    case H_SPUoff2:
      psf2log_ps1_trace("write KOFF hi val=%04X",val);
      SoundOff(16,24,val);
	// printf("Boop: %08x: %04x\n",reg,val);
      break;
    //-------------------------------------------------//
     ...
     case 0xC:                                          // get adsr vol
      {
       ...
       u16 result;
       result=(u16)(s_chan[ch].ADSRX.EnvelopeVol>>16);
       if(s_chan[ch].bStop && result>0)
        {
         u16 release_bias=(u16)(ch&0x1f);
         result=(result>release_bias)?(u16)(result-release_bias):0;
        }
       psf2log_ps1_trace("read ENVX ch=%02d raw=%04X ret=%04X bOn=%d bNew=%d bStop=%d start=%05X curr=%05X",
        ch,(u16)(s_chan[ch].ADSRX.EnvelopeVol>>16),result,s_chan[ch].bOn,s_chan[ch].bNew,s_chan[ch].bStop,
        s_chan[ch].pStart!=nullptr ? (unsigned)((s_chan[ch].pStart-spuMemC)>>3) : 0u,
        s_chan[ch].pCurr!=nullptr && s_chan[ch].pCurr!=(u8*)-1 ? (unsigned)((s_chan[ch].pCurr-spuMemC)>>3) : 0u);
       return result;
      }
 ...
 switch(r)
  {
    ...
    case H_SPUMute1:
     psf2log_ps1_trace("read ENDX lo ret=%04X mask=%06X",(unsigned)(psf2log_ps1_endx_mask&0xffffu),(unsigned)psf2log_ps1_endx_mask);
     return (u16)(psf2log_ps1_endx_mask&0xffffu);

    case H_SPUMute2:
     psf2log_ps1_trace("read ENDX hi ret=%04X mask=%06X",(unsigned)((psf2log_ps1_endx_mask>>16)&0xffu),(unsigned)psf2log_ps1_endx_mask);
     return (u16)((psf2log_ps1_endx_mask>>16)&0xffu);
...
{
 int NP;
 unsigned int locked_pitch;
 if(val>0x3fff) NP=0x3fff;                             // get pitch val
 else           NP=val;
 if(psf2log_peops_get_pitch_lock_value((unsigned int)ch,&locked_pitch))
  NP=(int)locked_pitch;
