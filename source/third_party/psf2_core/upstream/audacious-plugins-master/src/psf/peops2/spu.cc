...
REVERBInfo2      rvb[2];
extern unsigned short spuCtrl2[2];
unsigned int     psf2log_peops2_reverb_force_on_masks[2];
unsigned int     psf2log_peops2_reverb_force_off_masks[2];
unsigned int     psf2log_peops2_noise_force_on_masks[2];
unsigned int     psf2log_peops2_noise_force_off_masks[2];
unsigned int     psf2log_peops2_pmod_force_on_masks[2];
unsigned int     psf2log_peops2_pmod_force_off_masks[2];
static unsigned int psf2log_peops2_adsr_force_masks[2];
static int psf2log_peops2_adsr_freeze;
static unsigned int psf2log_peops2_pitch_lock_masks[2];
static unsigned int psf2log_peops2_pitch_lock_values[MAXCHAN];
static unsigned int psf2log_peops2_volume_lock_masks[2][2];
static unsigned int psf2log_peops2_volume_lock_values[2][MAXCHAN];
static int psf2log_peops2_mute_gain[MAXCHAN];
static int psf2log_peops2_mute_gain_init;
static unsigned int psf2log_peops2_timbre_solo_enabled;
static unsigned int psf2log_peops2_timbre_solo_starts[64];
static unsigned int psf2log_peops2_timbre_solo_loops[64];
static unsigned int psf2log_peops2_timbre_solo_flags[64];
static unsigned int psf2log_peops2_timbre_solo_key_count;
static int psf2log_peops2_timbre_gain[MAXCHAN];
static unsigned int psf2log_peops2_timbre_keyed_start[MAXCHAN];
static unsigned int psf2log_peops2_timbre_keyed_flags[MAXCHAN];
static unsigned char psf2log_peops2_timbre_allowed_latched[MAXCHAN];
static int psf2log_peops2_timbre_gain_init;

static int psf2log_peops2_effective_noise(int ch);
static int psf2log_peops2_effective_pmod_target(int ch);
static int psf2log_peops2_timbre_start_allowed(unsigned int start_addr, unsigned int flags);

static void psf2log_peops2_init_mute_gains(void)
{
 int ch;
 if(psf2log_peops2_mute_gain_init) return;
 for(ch=0;ch<MAXCHAN;ch++) psf2log_peops2_mute_gain[ch]=4096;
 psf2log_peops2_mute_gain_init=1;
}

static int psf2log_peops2_next_mute_gain(unsigned int ch, int muted)
{
 const int step=32;
 ...
 gain=psf2log_peops2_mute_gain[ch];
 if(gain<target)
  {
   gain+=step;
 ...
 psf2log_peops2_mute_gain[ch]=gain;
 return gain;
}

extern "C" void psf2log_peops2_set_mute_masks(unsigned int core0_mask, unsigned int core1_mask)
{
 unsigned int masks[2]={core0_mask,core1_mask};
 int ch;

 psf2log_peops2_init_mute_gains();
 for(ch=0;ch<MAXCHAN;ch++)
  {
   s_chan[ch].iMute=(masks[ch/24]&(1u<<(ch%24)))?1:0;
  }
}
...
extern "C" void psf2log_peops2_set_timbre_solo(int enabled, const unsigned int *starts, const unsigned int *loops, const unsigned int *flags, unsigned int count)
{
 ...
 psf2log_peops2_timbre_solo_enabled=enabled?1u:0u;
 psf2log_peops2_timbre_solo_key_count=count;
 for(i=0;i<count;i++)
  {
    psf2log_peops2_timbre_solo_starts[i]=starts!=nullptr?(starts[i]&0x000fffffu):0u;
    psf2log_peops2_timbre_solo_loops[i]=loops!=nullptr?(loops[i]&0x000fffffu):0u;
    psf2log_peops2_timbre_solo_flags[i]=flags!=nullptr?(flags[i]&0x0cu):0u;
  }
 for(;i<64;i++)
  {
   psf2log_peops2_timbre_solo_starts[i]=0u;
    psf2log_peops2_timbre_solo_loops[i]=0u;
    psf2log_peops2_timbre_solo_flags[i]=0u;
  }
 for(ch=0;ch<MAXCHAN;ch++)
  psf2log_peops2_timbre_allowed_latched[ch]=
   psf2log_peops2_timbre_start_allowed(
    psf2log_peops2_timbre_keyed_start[ch],
    psf2log_peops2_timbre_keyed_flags[ch])?1u:0u;
}

static unsigned int psf2log_peops2_ptr_addr(const unsigned char *ptr)
{
 if(ptr==nullptr || ptr<spuMemC) return 0xffffffffu;
 return (unsigned int)(((ptr-spuMemC)>>1)&0x000fffffu);
}

static unsigned int psf2log_peops2_timbre_channel_flags(int ch)
{
 unsigned int flags=0;
 if(psf2log_peops2_effective_noise(ch)) flags|=0x04u;
 if(psf2log_peops2_effective_pmod_target(ch)) flags|=0x08u;
 return flags;
}
...
{
 unsigned int i;
 if(!psf2log_peops2_timbre_solo_enabled || psf2log_peops2_timbre_solo_key_count==0) return 1;
 if(start_addr==0xffffffffu) return 0;
 for(i=0;i<psf2log_peops2_timbre_solo_key_count;i++)
  {
   unsigned int selected_start=psf2log_peops2_timbre_solo_starts[i];
   if(selected_start==start_addr && psf2log_peops2_timbre_solo_flags[i]==flags) return 1;
  }
 return 0;
}

static void psf2log_peops2_init_timbre_gains(void)
{
 unsigned int ch;
 if(psf2log_peops2_timbre_gain_init) return;
 for(ch=0;ch<MAXCHAN;ch++)
  {
   psf2log_peops2_timbre_gain[ch]=4096;
   psf2log_peops2_timbre_keyed_start[ch]=0xffffffffu;
   psf2log_peops2_timbre_keyed_flags[ch]=0u;
   psf2log_peops2_timbre_allowed_latched[ch]=1u;
  }
 psf2log_peops2_timbre_gain_init=1;
}

static int psf2log_peops2_next_timbre_gain(int ch)
{
 const int step=64;
 int target;
 int gain;
 psf2log_peops2_init_timbre_gains();
 if(ch<0 || ch>=MAXCHAN) return 0;
 target=psf2log_peops2_timbre_allowed_latched[ch]?4096:0;
 gain=psf2log_peops2_timbre_gain[ch];
 if(gain<target)
  {
   gain+=step;
   if(gain>target) gain=target;
 ...
 psf2log_peops2_timbre_gain[ch]=gain;
 return gain;
}

extern "C" void psf2log_peops2_set_reverb_override_masks(unsigned int core0_on, unsigned int core1_on, unsigned int core0_off, unsigned int core1_off)
{
 psf2log_peops2_reverb_force_on_masks[0]=core0_on&0x00ffffffu;
 psf2log_peops2_reverb_force_on_masks[1]=core1_on&0x00ffffffu;
 psf2log_peops2_reverb_force_off_masks[0]=core0_off&0x00ffffffu;
 psf2log_peops2_reverb_force_off_masks[1]=core1_off&0x00ffffffu;
}

extern "C" void psf2log_peops2_set_noise_override_masks(unsigned int core0_on, unsigned int core1_on, unsigned int core0_off, unsigned int core1_off)
{
 psf2log_peops2_noise_force_on_masks[0]=core0_on&0x00ffffffu;
 psf2log_peops2_noise_force_on_masks[1]=core1_on&0x00ffffffu;
 psf2log_peops2_noise_force_off_masks[0]=core0_off&0x00ffffffu;
 psf2log_peops2_noise_force_off_masks[1]=core1_off&0x00ffffffu;
}

extern "C" void psf2log_peops2_set_pmod_override_masks(unsigned int core0_on, unsigned int core1_on, unsigned int core0_off, unsigned int core1_off)
{
 psf2log_peops2_pmod_force_on_masks[0]=core0_on&0x00ffffffu;
 psf2log_peops2_pmod_force_on_masks[1]=core1_on&0x00ffffffu;
 psf2log_peops2_pmod_force_off_masks[0]=core0_off&0x00ffffffu;
 psf2log_peops2_pmod_force_off_masks[1]=core1_off&0x00ffffffu;
}
 ...
 for(ch=0;ch<MAXCHAN;ch++)
  {
   ...
   unsigned int voice=ch%24u;
   if(core<2 && (psf2log_peops2_adsr_force_masks[core]&(1u<<voice))!=0)
    {
...
extern "C" int psf2log_peops2_get_adsr_force(unsigned int ch)
{
 unsigned int core;
 unsigned int voice;
 if(ch>=MAXCHAN) return 0;
 core=ch/24u;
 voice=ch%24u;
 if(core>=2) return 0;
 return (psf2log_peops2_adsr_force_masks[core]&(1u<<voice))!=0;
}

extern "C" void psf2log_peops2_set_adsr_freeze(int enabled)
{
 psf2log_peops2_adsr_freeze=enabled?1:0;
}

extern "C" int psf2log_peops2_get_adsr_freeze(void)
{
 return psf2log_peops2_adsr_freeze;
}

static void psf2log_peops2_restore_pitch_from_raw(unsigned int ch)
{
 int NP;
...
extern "C" void psf2log_peops2_restore_pitch(unsigned int core, unsigned int voice)
{
 if(core>=2 || voice>=24) return;
 psf2log_peops2_restore_pitch_from_raw((core*24u)+voice);
}

extern "C" void psf2log_peops2_set_pitch(unsigned int core, unsigned int voice, unsigned int value)
{
 unsigned int ch;
 if(core>=2 || voice>=24) return;
 if(value>0x3fffu) value=0x3fffu;
 ch=(core*24u)+voice;
 s_chan[ch].iRawPitch=(int)value;
 psf2log_peops2_restore_pitch_from_raw(ch);
}

extern "C" void psf2log_peops2_set_pitch_lock(unsigned int core, unsigned int voice, int enabled, unsigned int value)
{
 unsigned int bit;
 if(core>=2 || voice>=24) return;
 if(value>0x3fffu) value=0x3fffu;
 ...
 if(enabled)
  {
   psf2log_peops2_pitch_lock_values[(core*24u)+voice]=value;
   psf2log_peops2_pitch_lock_masks[core]|=bit;
...
extern "C" int psf2log_peops2_get_pitch_lock_value(unsigned int ch, unsigned int *out_value)
{
 unsigned int core;
 unsigned int voice;
 if(ch>=MAXCHAN || out_value==NULL) return 0;
 core=ch/24u;
 voice=ch%24u;
 if(core>=2) return 0;
 if((psf2log_peops2_pitch_lock_masks[core]&(1u<<voice))==0) return 0;
 *out_value=psf2log_peops2_pitch_lock_values[ch]&0x3fffu;
 return 1;
}

static int psf2log_peops2_effective_volume_from_raw(unsigned int value)
{
 short vol=(short)(value&0xffffu);
 if(vol&0x8000)
  {
   short sInc=1;
...
extern "C" void psf2log_peops2_set_volume(unsigned int core, unsigned int voice, unsigned int side, unsigned int value)
{
 unsigned int ch;
 int effective;
 if(core>=2 || voice>=24 || side>=2) return;
 ch=(core*24u)+voice;
 value&=0xffffu;
 effective=psf2log_peops2_effective_volume_from_raw(value);
 if(side==0)
  {
   s_chan[ch].iLeftVolRaw=(int)(short)value;
   s_chan[ch].iLeftVolume=effective;
  }
 else
  {
   s_chan[ch].iRightVolRaw=(int)(short)value;
   s_chan[ch].iRightVolume=effective;
  }
}

extern "C" void psf2log_peops2_set_volume_lock(unsigned int core, unsigned int voice, unsigned int side, int enabled, unsigned int value)
{
 unsigned int bit;
 if(core>=2 || voice>=24 || side>=2) return;
 bit=1u<<voice;
 if(enabled)
  {
   psf2log_peops2_volume_lock_values[side][(core*24u)+voice]=value&0xffffu;
   psf2log_peops2_volume_lock_masks[side][core]|=bit;
...
{
 if(core>=2 || voice>=24 || side>=2 || out_value==NULL) return 0;
 if((psf2log_peops2_volume_lock_masks[side][core]&(1u<<voice))==0) return 0;
 *out_value=psf2log_peops2_volume_lock_values[side][(core*24u)+voice]&0xffffu;
 return 1;
}

extern "C" void psf2log_peops2_set_adsr_value(unsigned int core, unsigned int voice, unsigned int field, unsigned int value)
{
 unsigned int ch;
 if(core>=2 || voice>=24) return;
 ch=(core*24u)+voice;
 switch(field)
 {
  case 0: s_chan[ch].ADSRX.AttackRate=(int)(value&0x7fu); break;
  case 1: s_chan[ch].ADSRX.DecayRate=(int)(value&0x0fu); break;
  case 2: s_chan[ch].ADSRX.SustainLevel=(int)(value&0x0fu); break;
  case 3: s_chan[ch].ADSRX.SustainRate=(int)(value&0x7fu); break;
  case 4: s_chan[ch].ADSRX.ReleaseRate=(int)(value&0x1fu); break;
  default: break;
 }
}

extern "C" void psf2log_peops2_set_noise_clock(unsigned int core, unsigned int value)
{
 if(core>=2) return;
 if(value>0x3fu) value=0x3fu;
 spuCtrl2[core]=(unsigned short)((spuCtrl2[core]&~0x3f00u)|(value<<8));
}

extern "C" void psf2log_peops2_set_reverb_value(unsigned int core, unsigned int side, unsigned int value)
{
 if(core>=2) return;
 value&=0xffffu;
 if(side==0) rvb[core].VolLeft=(short)value;
 else if(side==1) rvb[core].VolRight=(short)value;
}

static int psf2log_peops2_effective_noise(int ch)
{
 unsigned int core=(unsigned int)ch/24u;
 unsigned int voice=(unsigned int)ch%24u;
 unsigned int bit=1u<<voice;
 if((psf2log_peops2_noise_force_off_masks[core]&bit)!=0) return 0;
 if((psf2log_peops2_noise_force_on_masks[core]&bit)!=0) return 1;
...
static int psf2log_peops2_effective_pmod_target(int ch)
{
 ...
 unsigned int voice=(unsigned int)ch%24u;
 unsigned int bit=1u<<voice;
 if((psf2log_peops2_pmod_force_off_masks[core]&bit)!=0) return 0;
 if((psf2log_peops2_pmod_force_on_masks[core]&bit)!=0) return 1;
 return s_chan[ch].bFMod==1;
}

static int psf2log_peops2_effective_pmod_source(int ch)
{
 if((ch%24)==23) return 0;
...
static inline void StartSound(int ch)
{
 ...
 StartREVERB(ch);
 psf2log_peops2_init_timbre_gains();
 timbre_start=psf2log_peops2_ptr_addr(s_chan[ch].pStart);
 timbre_flags=psf2log_peops2_timbre_channel_flags(ch);
 timbre_allowed=psf2log_peops2_timbre_start_allowed(timbre_start,timbre_flags);
 psf2log_peops2_timbre_keyed_start[ch]=timbre_start;
 psf2log_peops2_timbre_keyed_flags[ch]=timbre_flags;
 psf2log_peops2_timbre_allowed_latched[ch]=timbre_allowed?1u:0u;
 psf2log_peops2_timbre_gain[ch]=timbre_allowed?4096:0;
...
//           else                                        // else adjust
         ...
         if(psf2log_peops2_effective_noise(ch))
          {
           ...
           fa=s_chan[ch].iOldNoise+((fa-s_chan[ch].iOldNoise)/((0x003f-((spuCtrl2[ch/24]&0x3f00)>>8))+1));
           if(fa>32767L)  fa=32767L;
           if(fa<-32767L) fa=-32767L;
...
*/
         ...
         {
         int pmod_source=psf2log_peops2_effective_pmod_source(ch);
         ...
         if(1)
          {
           ...
           {
            int old_sval=s_chan[ch].sval;
            int mute_gain=psf2log_peops2_next_mute_gain(ch,s_chan[ch].iMute);
            int timbre_gain=psf2log_peops2_next_timbre_gain(ch);
            s_chan[ch].sval=(s_chan[ch].sval*mute_gain)>>12;
            s_chan[ch].sval=(s_chan[ch].sval*timbre_gain)>>12;
            if(iUseMain)
             {
           ...
           if(s_chan[ch].bRVBActive ||
              (psf2log_peops2_reverb_force_on_masks[ch/24]&(1u<<(ch%24)))!=0) StoreREVERB(ch,0);
            s_chan[ch].sval=old_sval;
           }
          }
         }
...
//   s_chan[i].hMutex=CreateMutex(nullptr,false,nullptr);
   ...
   s_chan[i].iMute=0;
   psf2log_peops2_mute_gain[i]=4096;
   psf2log_peops2_timbre_gain[i]=4096;
   psf2log_peops2_timbre_keyed_start[i]=0xffffffffu;
   psf2log_peops2_timbre_keyed_flags[i]=0u;
   psf2log_peops2_timbre_allowed_latched[i]=1u;
   s_chan[i].iIrqDone=0;
