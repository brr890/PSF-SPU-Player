...
void SetVolumeL(unsigned char ch,short vol)            // LEFT VOLUME
{
 ...
 if(psf2log_peops2_get_volume_lock_value((unsigned int)ch/24u,(unsigned int)ch%24u,0u,&locked_volume))
  vol=(short)(locked_volume&0xffffu);
...
{
 unsigned int locked_volume;
 if(psf2log_peops2_get_volume_lock_value((unsigned int)ch/24u,(unsigned int)ch%24u,1u,&locked_volume))
  vol=(short)(locked_volume&0xffffu);
...
void SetPitch(int ch,unsigned short val)               // SET PITCH
{
 ...
 if(psf2log_peops2_get_pitch_lock_value((unsigned int)ch,&locked_pitch))
  NP=(int)locked_pitch;
